...
async function sendTelegramDocument(t, c, docUrl, cap = '', k = null, p = 'HTML') {
    ...
    try {
        ...
        return result;
    } catch (e) { console.error('TG sendDocument err:', e); return null; }
}
...
// ** ✨ Create QRIS (FINAL FIX STABILITAS) ✨ **
async function createQrisAndConfirm(env, user, nominal) {
    ...
    try {
        console.log(`[User ${userId}] Requesting QRIS for amount: ${finalNominal} (Nom: ${nominal}, Random: ${randomAddition})`);
        ...
        if (data.status === "success") { // ** KEMBALI KE LOGIKA AWAL YANG STABIL **
            const qrisUrl = data.data.download_url; // Path awal yang stabil
            const transactionId = data.data["kode transaksi"]; // Path awal yang stabil
            ...
            if (!saveStatus) { console.error(`[User ${userId}] Failed to save pending payment to KV.`); throw new Error("Gagal menyimpan data deposit sementara."); }
            ...
            // ** FIX QRIS: Gunakan sendDocument (Lebih andal) **
            const sent = await sendTelegramDocument(env.BOT_TOKEN, userId, qrisUrl, caption, keyboard);
...
router.post('/', async (request, env, ctx) => {
    try {
        ...
        if (update.message) {
            ...
            // ** PRIORITAS 1: Perintah /admin (Fix) **
            if (text.startsWith('/admin') && user.id.toString() === env.ADMIN_ID) {
                 if (session) userSessions.delete(user.id); // Hapus sesi untuk fresh start
            ...
            // Prioritas 3: Sesi Deposit (Nominal input)
            ...
            // Abaikan teks biasa dari non-admin di luar sesi
             ...
             if (usersData[user.id.toString()]?.is_banned) { await answerCallbackQuery(env.BOT_TOKEN, update.callback_query.id, "🚫 Akun Anda diblokir", true); responseObj = null; }
                 ...
                 else if (cbData.startsWith('cancel_payment'))      responseObj = await handleCancelPayment(update, env);
