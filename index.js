...
// ** ✨ Handle /start & Kembali (Teks Persis Sesuai Request) ✨ **
...
async function createQrisAndConfirm(env, user, nominal) { /* ... */ const rand = getRandomAmount(env); const final = nominal + rand; const userId = user.id; try { console.log(`[User ${userId}] Requesting QRIS for amount: ${final}`); const resp = await fetch(`${env.API_CREATE_URL}?amount=${final}&qrisCode=${env.QRIS_CODE}`); console.log(`[User ${userId}] QRIS API Status: ${resp.status}`); if (!resp.ok) { const errTxt = await resp.text(); console.error(`[User ${userId}] QRIS API Error Response Text:`, errTxt); throw new Error(`API Pembayaran mengembalikan status ${resp.status}`); } const data = await resp.json(); console.log(`[User ${userId}] QRIS API Success Response JSON:`, JSON.stringify(data, null, 2)); const qrisUrl = data?.data?.download_url || data?.download_url || data?.data?.qris_url || data?.qris_url; const transactionId = data?.data?.["kode transaksi"] || data?.data?.transaction_id || data?.transaction_id || data?.id; if (!qrisUrl || !transactionId) { console.error(`[User ${userId}] Missing qrisUrl or transactionId in API response structure.`); throw new Error("Data QRIS tidak lengkap dari API pembayaran."); } console.log(`[User ${userId}] Extracted QRIS URL: ${qrisUrl}`); console.log(`[User ${userId}] Extracted Transaction ID: ${transactionId}`); const pData = { nominal, finalNominal: final, transactionId: transactionId, timestamp: new Date(), status: "pending", messageId: null }; const saveStatus = await savePendingPayment(env.BOT_DB, userId, pData); if (!saveStatus) { console.error(`[User ${userId}] Failed to save pending payment to KV.`); throw new Error("Gagal menyimpan data deposit sementara."); } const kb = { inline_keyboard: [[{ text: "✅ Saya Sudah Transfer", callback_data: `confirm_payment_${transactionId}` }, { text: "❌ Batalkan", callback_data: "cancel_payment" }]] }; const cap = `⏳ ===「 <b>INSTRUKSI DEPOSIT</b> 」=== ⏳\n\nTransfer <b>TEPAT</b> <code>Rp ${formatNumber(final)}</code>\n──────────────────\nID: <code>${transactionId}</code>\nNominal: <code>Rp ${formatNumber(nominal)}</code>\nKode Unik: <code>Rp ${rand}</code>\n<b>TOTAL:</b> 👉 <h1><code>Rp ${formatNumber(final)}</code></h1> 👈\n──────────────────\nScan QRIS.\nBatas waktu: ⏱️ <b>10 menit</b>.\n\nKlik "✅ Sudah Transfer" <b>SETELAH</b> transfer.`; const sent = await sendTelegramPhoto(env.BOT_TOKEN, userId, qrisUrl, cap, kb); if (sent?.ok) { pData.messageId = sent.result.message_id; await savePendingPayment(env.BOT_DB, userId, pData); console.log(`[User ${userId}] QRIS Photo sent successfully, msg_id: ${pData.messageId}`); } else { console.error(`[User ${userId}] Failed to send QRIS photo. TG Response:`, JSON.stringify(sent)); await sendTelegramMessage(env.BOT_TOKEN, userId, "❌ Gagal menampilkan gambar QRIS saat ini. Mohon ulangi proses deposit."); await removePendingPayment(env.BOT_DB, userId); return; } const admMsg = `⏳ Depo Pending: @${user.username || userId} | ${transactionId} | Rp ${formatNumber(final)}`; await sendTelegramMessage(env.BOT_TOKEN, env.ADMIN_ID, admMsg); } catch (e) { console.error(`[User ${userId}] Create QRIS overall err:`, e); let userErrMsg = "❌ Terjadi kesalahan saat membuat QRIS. Masalah pada sistem pembayaran."; await sendTelegramMessage(env.BOT_TOKEN, userId, userErrMsg); try { await removePendingPayment(env.BOT_DB, userId); } catch (removeErr) {} } }
...
// ** Perintah Admin ini akan dipanggil oleh router.post **
...
// ** Fungsi handleDisplayAdminMenu (Untuk tombol kembali) **
